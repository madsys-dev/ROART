...
class Coordinator {
   ...
   public:
    ...
    void worker(ART_ROWEX::Tree* art, int workerid, Result* result,
                Benchmark* b) {
...
#define sync_latency(a, b, count)                                        \
        ...
        while (done == 0) {
            ...
            switch (op) {
                ...
                case INSERT:
                    ...
                    if (res == ART_ROWEX::Tree::OperationResults::Success) {
                        count++;
                    }
                    break;
                case REMOVE:
    ...
    void run() {
        ...
        for (int i = 0; i < conf.num_threads; i++) {
            pid[i] = new std::thread(&Coordinator::worker, this, art, i,
                                     &results[i], benchmark);
        }
