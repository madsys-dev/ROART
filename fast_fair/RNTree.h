...
void init_pmem() {
    // create pool
    const char *pool_name = "/mnt/pmem0/matianmao/rntree.data";
    const char *layout_name = "rntree";
    size_t pool_size = 64LL * 1024 * 1024 * 1024; // 64GB

    if (access(pool_name, 0)) {
        pmem_pool = pmemobj_create(pool_name, layout_name, pool_size, 0666);
        if (pmem_pool == nullptr) {
            std::cout << "[RNTREE]\tcreate fail\n";
            assert(0);
        }
        std::cout << "[RNTREE]\tcreate\n";
    } else {
        pmem_pool = pmemobj_open(pool_name, layout_name);
        std::cout << "[RNTREE]\topen\n";
    }
    std::cout << "[RNTREE]\topen pmem pool successfully\n";
}
...
void *allocate(size_t size) {
    ...
    int ret = pmemobj_zalloc(pmem_pool, &ptr, sizeof(char) * size,
                             TOID_TYPE_NUM(char));
    if (ret) {
        std::cout << "[RNTREE]\tallocate btree successfully\n";
        assert(0);
    }
...
template <typename K, typename V, int size> class Node {
  public:
    virtual bool isLeaf() = 0;
};

template <typename K, typename V, int size> class LN : public Node<K, V, size> {
  ...
  public:
    ...
    inline uint64_t get_version() { return version >> VERSION_SHIFT; }
    inline uint64_t set_version(uint64_t _v) {
        version = (_v << VERSION_SHIFT) | (version & META_MASK);
    }

    inline uint64_t unlock_version(uint64_t _v) { return _v & (~LOCK_MASK); }
    inline uint64_t locked_version(uint64_t _v) { return _v | LOCK_MASK; }
    ...
    inline int allocate_entry() {
        //        std::cout<<"leaf: "<<this<<" entry is "<<entry<<"\n";
        uint64_t e = entry;
        if (e >= LN_SIZE) {
            do {
                //                std::cout<<"pause\n";
    ...
    void lock() {
        uint64_t _v = version;
        while (!__sync_bool_compare_and_swap(&version, unlock_version(_v),
                                             locked_version(_v))) {
            _v = version;
        }
...
template <typename K, typename V, int size> class IN : public Node<K, V, size> {
  public:
  ...
  public:
    ...
    inline node_t *find(K key) { return children_ptrs[binary_search(key)]; }
...
template <typename K, typename V, int size> class Btree {
    typedef Node<K, V, size> node_t;
    typedef IN<K, V, size> inner_node_t;
    ...
    node_t *split_inner_node(inner_node_t *node) {
        ...
        for (int i = 0; i < size / 2; i++) {
            ...
            if (successor->children_ptrs[i]->isLeaf() == false) {
                ((inner_node_t *)(successor->children_ptrs[i]))->parent =
                    successor;
            }
    ...
    void insertInnerNode(inner_node_t *parent, leaf_node_t *leaf,
                         leaf_node_t *newleaf, K sep) {
        bool needsplit = true;
    ...
    void htmTreeUpdate(inner_node_t *parent, leaf_node_t *leaf,
                       leaf_node_t *next, K sep) {
        speculative_lock_t::scoped_lock _lock;
    ...
    void generateNextLeaf(leaf_node_t *leaf, K &sep) {
        ...
        PMEMoid ptr;
        pmemobj_zalloc(pmem_pool, &ptr, sizeof(leaf_node_t),
                       TOID_TYPE_NUM(char));
        leaf_node_t *next = new (pmemobj_direct(ptr)) leaf_node_t;

        pmemobj_zalloc(pmem_pool, &ptr, sizeof(leaf_node_t),
                       TOID_TYPE_NUM(char));
        leaf_node_t *log = new (pmemobj_direct(ptr)) leaf_node_t;
        ...
        leaf->persist_entry = leaf->entry = leaf->dslot[0] = leaf->slot[0] =
            split;
        next->persist_entry = next->entry = next->dslot[0] = next->slot[0] =
            log->slot[0] - split;
    ...
    bool modify(K key, V value, double latency_breaks[3], bool remove = false) {
        ...
        while (true) {
            ...
            // printf("%lf %lf %lf\n", latency_breaks[0], latency_breaks[1],
            // latency_breaks[2]);
#endif
  ...
  public:
    Btree() {
        ...
        TX_BEGIN(pmem_pool) {
            ...
            PMEMoid ptr =
                pmemobj_tx_zalloc(sizeof(leaf_node_t), TOID_TYPE_NUM(char));
            root = (node_t *)(new (pmemobj_direct(ptr)) leaf_node_t);
            flush_data((void *)root, sizeof(leaf_node_t));
        }
        TX_END
        anchor = (leaf_node_t *)root; // Never changed.
        printf("[RNTREE]new rtm R tree\n");
    }
    ...
    bool remove(K key) { return modify(key, V(0), nullptr, true); }
