...
public:
    ...
    bool remove_rebalancing(btree *bt, entry_key_t key,
        ...
        if (hdr.leftmost_ptr) ++total_num_entries;
        ...
        if (total_num_entries > cardinality - 1) {  // Redistribution
            ...
            if (num_entries < left_num_entries) {  // left -> right
                if (hdr.leftmost_ptr == nullptr) {
                    ...
                    clflush((char *)&(left_sibling->records[m].ptr),
                            sizeof(char *));

                    left_sibling->hdr.last_index = m - 1;
                    clflush((char *)&(left_sibling->hdr.last_index),
                            sizeof(int16_t));

                    parent_key = records[0].key;
                } else {
                    insert_key(deleted_key_from_parent,
                               (char *)hdr.leftmost_ptr, &num_entries);
                ...
                if (hdr.leftmost_ptr == nullptr) {
                    ...
                    for (int i = num_dist_entries; records[i].ptr != NULL;
                         i++) {
                ...
                if (left_sibling == ((page *)bt->root)) {
                    page *new_root = new page(left_sibling, parent_key,
    ...
    void print() {
        ...
        if (hdr.leftmost_ptr != NULL) printf("%x ", hdr.leftmost_ptr);
