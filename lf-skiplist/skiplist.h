...
#define CACHE_LINES_PER_NV_NODE                                                \
    3 // TODO does nv-jemalloc need to be aware of this?

const int max_level = 20; // one cache-line node; use 13 for two cache-line
// nodes
...
inline void flush_and_try_unflag(PVOID *target) {
    ...
    if (is_marked_ptr_cache((UINT_PTR)value)) {
        ...
        CAS_PTR((volatile PVOID *)target, value,
                (PVOID)unmark_ptr_cache((UINT_PTR)value));
    }
}
...
inline PVOID link_and_persist(PVOID *target, PVOID oldvalue, PVOID value) {
    ...
    PVOID res;
    res = CAS_PTR(target, (PVOID)oldvalue,
                  (PVOID)mark_ptr_cache((UINT_PTR)value));
    ...
    flush_data(target, sizeof(PVOID));
    CAS_PTR((volatile PVOID *)target, (PVOID)mark_ptr_cache((UINT_PTR)value),
            (PVOID)value);
    return res;
}
...
struct node_t {
    ...
    node_t *next[max_level +
                 2]; // in our allocator, we will be working with chunks of the
    // same size, so every node should have the same size
    uint32_t toplevel;
    uint8_t node_flags;
};
