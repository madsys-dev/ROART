...
//simple search, does not do any cleanup, only sets the successors of the retrieved node
    ...
    inline int mark_node_pointers(node_t *node) {
        ...
        for (i = node->toplevel - 1; i >= 1; i--) {
            do {
                ...
                success = (CAS_PTR((PVOID *) &node->next[i], UNMARKED_PTR(next_node), MARKED_PTR(next_node)) ==
                           UNMARKED_PTR(next_node)) ? 1 : 0;
        ...
        do {
            ...
            success = ((node_t *) link_and_persist((PVOID *) &node->next[0], UNMARKED_PTR(next_node),
                                                   MARKED_PTR(next_node)) == UNMARKED_PTR(next_node)) ? 1 : 0;
        } while (success == 0);
        return success;
    }
    ...
    int skiplist_insert(skiplist_t *sl, skey_t key, svalue_t val) {
        ...
        flush_data((void *)to_insert, sizeof(node_t)); //we persist the newly allocated data in new_node;


        if ((node_t *)link_and_persist((PVOID *) &(preds[0]->next[0]), (PVOID) UNMARKED_PTR_ALL(succs[0]),
    ...
    void skiplist_update(skiplist_t *sl, skey_t key, svalue_t val) {
        ...
        if(!PTR_IS_MARKED(node_to_update)){
            ...
            TX_BEGIN(pmem_pool){
                PMEMoid nv = pmemobj_tx_zalloc(strlen(val) + 1, TOID_TYPE_NUM(char));
