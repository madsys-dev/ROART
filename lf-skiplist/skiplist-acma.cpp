...
//            if (rand() % 100 < 50) { // TODO replace the mod with something
//            else?
//                level++;
//            } else {
//                break;
...
//            // write_data_wait((void*)node_to_delete,
//            CACHE_LINES_PER_NV_NODE);
//            // no need for the above; we either make sure the bottom unlink is
//            // already visible, or it is already in the flushbuffer, which
//            will be
...
//        flush_data(
//                (void *)to_insert,
//                sizeof(node_t)); // we persist the newly allocated data in
//                new_node;
//
...
//            // (PVOID)to_insert) != UNMARKED_PTR(succs[0])) { failed to insert
//            the
//            // node, so I can actually free it if I want there's no chance
//            anyone
//            // has a reference to it, right? so I can just call the finalize
//            on it
//            // directly EpochReclaimObject(epoch, (void*) to_insert, NULL,
//            NULL,
//            // finalize_node);
//#ifdef USE_PMDK
//#ifdef FF_GC
