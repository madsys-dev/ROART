            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE),
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  ...
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      Key *key = key->make_leaf((char *) init_keys[i]->fkey, init_keys[i]->key_len,
                                                                init_keys[i]->value);
                                      tree.insert(key, t);
                                  }
                              });
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, RUN_SIZE),
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  ...
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      Key *key;
                                      if (ops[i] == OP_INSERT) {
                                          key = key->make_leaf((char *) keys[i]->fkey, keys[i]->key_len,
                                                               keys[i]->value);
                                          tree.insert(key, t);
                                          ...
                                          if (val->value != keys[i]->value) {
                                              std::cout << "[ART] wrong key read: " << val->value << " expected:"
                                                        << keys[i]->value << std::endl;
                                              throw;
                                          }
                                          ...
                                          Key *start = start->make_leaf((char *) keys[i]->fkey, keys[i]->key_len,
                                                                        keys[i]->value);
                                          tree.lookupRange(start, end, continueKey, results, resultsSize, resultsFound,
                                                           t);
                                      }
                                  }
                              });
                              ...
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      if (ops[i] == OP_INSERT) {
                                          ...
                                          uint64_t *ret = reinterpret_cast<uint64_t *> (bt->btree_search(
                                                  (char *) keys[i]->fkey));
                                          if (ret == NULL) {
                                          ...
                                          uint64_t buf[200];
                                          int resultsFound = 0;
                                          bt->btree_search_range((char *) keys[i]->fkey, (char *) maxKey.c_str(), buf,
                                                                 ranges[i], resultsFound);
                                      }
                                  }
                              });
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
...
void ycsb_load_run_randint(int index_type, int wl, int kt, int ap, int num_thread,
    ...
    if (index_type == TYPE_ART) {
                              ...
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  ...
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      if (ops[i] == OP_INSERT) {
                                          ...
                                          if (*val != keys[i]) {
                                              std::cout << "[ART] wrong key read: " << val << " expected:" << keys[i]
                                                        << std::endl;
                                              exit(1);
                                          }
                                      } else if (ops[i] == OP_SCAN) {
        ...
        auto t = new BwTree <uint64_t, uint64_t, KeyComparator, KeyEqualityChecker>{true, KeyComparator{1},
                              ...
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      if (ops[i] == OP_INSERT) {
                                          ...
                                          if (val == NONE) {
                                              std::cout << "[Level Hashing] wrong key read: " << *(uint64_t *) val
                              ...
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      if (ops[i] == OP_INSERT) {
                                          ...
                                          uint64_t *val = reinterpret_cast<uint64_t *>(const_cast<char *>(table->Get(
                                                  keys[i])));
                                          if (val == NULL) {
                              ...
                              [&](const tbb::blocked_range<uint64_t> &scope) {
                                  for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                                      if (ops[i] == OP_INSERT) {
                                          ...
                                          uint64_t *ret = reinterpret_cast<uint64_t *> (woart_search(t, keys[i],
                                                                                                     sizeof(uint64_t)));
                                          if (*ret != keys[i]) {
